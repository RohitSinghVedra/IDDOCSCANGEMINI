...
const performOCRSpace = async (imageFile) => {
  try {
    ...
    formData.append('language', 'eng'); // Use 'eng' for best results with mixed content
...
const extractPANData = (text) => {
  ...
  // CRITICAL FIX: Avoid matching "Father's Name" as the Name
  const namePatterns = [
    // Look for Name explicitly NOT preceded by Father/Pita
    /(?<!Father's\s|Father\s|Pita\s)(?:नाम|Name)[:\s/]+([A-Z][A-Z\s]{4,})/i,

    // Direct name extraction based on PAN layout:
    // Usually the first prominent uppercase text block that isn't "INCOME TAX" or "GOVT OF INDIA"
    // And is followed by "Father's Name"
    /([A-Z][A-Z\s]{4,})(?=\s+(?:Father|पिता))/i,

    // Fallback: Standard 2-3 word uppercase name pattern
...
const extractVoterIDData = (text) => {
  ...
  for (const pattern of namePatterns) {
    ...
    if (match) {
      let name = (match[1] || match[0]).trim();
      if (!name.match(/^(ELECTION|COMMISSION|IDENTITY|CARD|ELECTOR)$/i)) {
        data.name = name;
        break;
      }
    }
  ...
  const relativeNamePatterns = [
    /(?:Father'?s?|Husband'?s?)\s+Name[:\s]+([A-Z][A-Z\s]{2,})/i,
    /(?:Father|Husband)[:\s]+([A-Z][A-Z\s]{2,})/i
  ];

  for (const pattern of relativeNamePatterns) {
    const match = text.match(pattern);
    if (match) {
